...
(defn make
  ...
  (^doubles [^double x ^double y ^double z]
   (doto ^doubles (make-array Double/TYPE 3) (aset 0 x) (aset 1 y) (aset 2 z))))

(defmacro mutate! [v x y z]
  `(doto ~v (aset 0 ~x) (aset 1 ~y) (aset 2 ~z)))

(defn add ^doubles [^doubles v1 ^doubles v2]
  (make (+ (x v1) (x v2)) (+ (y v1) (y v2)) (+ (z v1) (z v2))))

(defn add! ^doubles [^doubles v1 ^doubles v2]
  (mutate! v1 (+ (x v1) (x v2)) (+ (y v1) (y v2)) (+ (z v1) (z v2))))
...
(defn cross ^doubles [^doubles u ^doubles v]
  (let [u0 (x u) u1 (y u) u2 (z u)
    ...
    (make (- (* u1 v2) (* u2 v1)) (- (* u2 v0) (* u0 v2)) (- (* u0 v1) (* u1 v0)))))
...
(defn near-zero? [v]
  (let [threshold 1e-8]
    (and (< (Math/abs (x v)) threshold)
         (< (Math/abs (y v)) threshold)
         (< (Math/abs (z v)) threshold))))
